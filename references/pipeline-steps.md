...
for clip in clips:
    result = model.transcribe(
        ...
        initial_prompt="<add your video's topic-specific vocabulary here>"
    )
...
**`initial_prompt` tip:** Add your video's topic-specific vocabulary to improve recognition accuracy. Include key nouns, proper nouns, and technical terms the speaker uses — the more specific, the better Whisper's kanji disambiguation.

## Step 3 — Transcript quality check (delegate)

```
delegate_task(
    goal="Fix transcript.json for a Japanese short-form video",
    context="""
    ...
    3. Fix domain-specific kanji misreads: cross-reference the initial_prompt vocabulary used in Step 2 and correct homophones or misread kanji that match the video's topic
...
```
delegate_task(
    goal="Create edit_plan.json for a Japanese short format video",
    context="""
